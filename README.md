...
3. 代理无法连接: 检查本地 `config.json` 是否生成成功，以及 `sing-box` 容器状态
4. 镜像构建卡住: `Dockerfile.subscribe` 已改为直接下载 GitHub 压缩包，不依赖 `apt` 安装 `git`
5. 宿主机网络异常: 将 `.env` 中的 `RUN_MODE` 改回 `proxy` 后重新执行 `./update-subscription.sh`
